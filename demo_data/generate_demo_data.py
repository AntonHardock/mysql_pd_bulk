...
## change output path to desired location
path = sys.argv[1]
n = int(sys.argv[2])

## function to create dummy data
def generate_dummy_data(n=1000, path=None):
        ...
        data.to_csv(path + "users.csv", index=False)
        ...
        data = pd.DataFrame(data)
        data.to_csv(path + "products.csv", index=False)
        ...
        data = pd.DataFrame(data)
        data.to_json(path + "reviews.json", orient="records", lines=True)

## choose number of rows in each table (n) and export 
generate_dummy_data(n=n, path=path)
